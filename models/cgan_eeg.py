...
class cGenerator_eeg(nn.Module):
...
class cDiscriminator_eeg(nn.Module):
...
class cGAN:
    def __init__(self,seq_len,features=3,n_critic=3,lr=5e-4,
        ...
        self.G = cGenerator_eeg(seq_len,features,g_hidden,label_dim=label_dim,latent_dim=latent_dim).to(self.device)
        self.D = cDiscriminator_eeg(seq_len,features,d_hidden,label_dim=label_dim).to(self.device)
