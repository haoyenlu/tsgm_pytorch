...
class cVAEConv(nn.Module):
    def __init__(self,seq_len,feat_dim,latent_dim,hidden_dim = 200,hidden_layer=[50,100],label_dim=5):
        super(cVAEConv,self).__init__()
        ...
        self.dense_mean = nn.Linear((self.seq_len + label_dim) // (2**len(self.hidden_layer)) * self.hidden_dim,self.latent_dim)
        self.dense_var = nn.Linear((self.seq_len + label_dim) // (2**len(self.hidden_layer)) * self.hidden_dim,self.latent_dim)
    ...
    def forward(self,X,label): # shape: (B,feats,seq_len) , label
        c = self.embedding(label).unsqueeze(1)
        _x = torch.cat([X,c.expand(-1,self.feat_dim,-1)],dim=2)
        z_mean, z_log_var = self.encoding(_x)
        z = self.sampling(z_mean,z_log_var)
        _z = torch.cat([z,c],dim=1)
        _z = self.decoding(_z)
